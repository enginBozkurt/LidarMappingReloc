#pragma once
// #include"utility.h" // 因为这个被mapOpt引用了，mapOpt也引用了utility.h，防止函数重复定义！
#include <ctime>
#include <cassert>
#include <cmath>
#include <utility>
...
using PointType = pcl::PointXYZI; // using xyz only. but a user can exchange the original bin encoding function (i.e., max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
...
public: 
    ...
    //     LIDAR_HEIGHT(LIDAR_HEIGHT), PC_NUM_RING(PC_NUM_RING),PC_NUM_SECTOR(PC_NUM_SECTOR),PC_MAX_RADIUS(PC_MAX_RADIUS){
    ...
    void saveScanContext(std::string filename);

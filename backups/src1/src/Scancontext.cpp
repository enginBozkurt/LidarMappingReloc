...
{
    std::vector<float> vec( _eigmat.data(), _eigmat.data() + _eigmat.size() ); // vec.begin(), vec.end()
    return vec;
} // eig2stdvec
...
std::pair<double, int> SCManager::distanceBtnScanContext( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    // originally there is no sector key
    MatrixXd vkey_sc1 = makeSectorkeyFromScancontext( _sc1 ); // return a 1xcolNum matrix
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<PointType> & _scan_down )
{
    ...
    // main
    const int NO_POINT = -1000; // initializing for getting max z
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0) // to save computation cost
    {
        ...
        // cout<<"Tree construction takes "<< t_tree_construction.toc()<<endl; //WHY???????????????
    }
...
// Save scan context as pics
void SCManager::saveScanContext(std::string filename){
    int numSC = polarcontexts_.size();  
    Eigen::MatrixXd tmp;  
    for( int i=0;i<numSC;i++){
        std::string filepath = filename + "/" + std::to_string(i) +".jpg";
        cv::Mat mat;
        tmp = polarcontexts_[i]/polarcontexts_[i].maxCoeff()*255.0; // normalize with the maximum
        // std::cout<<polarcontexts_[i]<<std::endl;
        cv::eigen2cv(tmp,mat);
        int rowN = tmp.rows();
        int colN = tmp.cols();
        if (!colN) cerr<<"zero column, wtf?"<<endl;
        cv::resize(mat,mat, cv::Size(200,200*rowN/colN),cv::INTER_CUBIC);
